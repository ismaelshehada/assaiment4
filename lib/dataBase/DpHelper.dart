...
class DatabaseHelper {
  ...
  void createDatabase(Database dbcreate, int versioncreate) async {
    await dbcreate.execute(
        "CREATE TABLE $taskTable($id INTEGER PRIMARY KEY AUTOINCREMENT, $taskName TEXT, $isComplate INTEGER)");
  }
  ...
  Future<List<Task>> getTaskListComplete() async {
    ...
    final data = tasksget.where((element) => element.isComplate == 1).toList();
    return data;
  }
  ...
  Future<List<Task>> getTaskListInComplete() async {
    ...
    print(tasksget.length);
    final data = tasksget.where((element) => element.isComplate == 0).toList();
    return data;
  }
}
